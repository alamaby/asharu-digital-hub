...
import { AddImageKeyForm, AddImageModelForm, ImageAccountForm, ReplaceImageKeyForm } from './ImageForms';
        ...
        renderItem={(id) => {
          ...
          return (
            <div>
              ...
              {p.slug === 'cloudflare' ? (
                <ImageAccountForm providerId={p.id} defaultValue={p.config?.account_id ?? ''} />
              ) : null}
            </div>
          );
        }}
...
export function ImageModelBoard({ providerId, providerName, models }: { providerId: string; providerName: string; models: ImageModel[] }) {
  ...
  async function handleToggle(m: ImageModel, active: boolean) {
    ...
    try {
      const r = await toggleImageModelActive(m.id, active);
      setNotice(r.ok
  ...
  return (
    <div className="space-y-2 rounded-lg border border-line p-3">
      <p className="text-sm font-semibold text-ink">Model · {providerName}</p>
          ...
          return (
            <div className="flex flex-wrap items-center justify-between gap-2">
              <p className="text-sm text-ink">
                #{m.priority} · {m.model_id} <span className="text-xs text-ink-muted">— {m.display_name}{m.is_default ? ' (default)' : ''} · used {m.usage_count}</span>
              </p>
              <label className="flex items-center gap-1 text-xs">
                <input
                  type="checkbox"
...
export function ImageKeyBoard({ providerId, providerName, providerSlug, keys }: { providerId: string; providerName: string; providerSlug: string; keys: ImageKey[] }) {
  ...
  async function handleToggle(k: ImageKey, active: boolean) {
    ...
    try {
      const r = await toggleImageKeyActive(k.id, active);
      setNotice(r.ok
  ...
  return (
    <div className="space-y-2 rounded-lg border border-line p-3">
      <p className="text-sm font-semibold text-ink">Keys · {providerName}</p>
      {keys.length === 0 ? <p className="text-xs text-ink-muted">Belum ada key.</p> : null}
      <SortableList
        items={keys.map((k) => ({ id: k.id }))}
        onReorder={(ids) => runReorder(ids, reorderImageKeys)}
        onSettled={settled}
        renderItem={(id) => {
