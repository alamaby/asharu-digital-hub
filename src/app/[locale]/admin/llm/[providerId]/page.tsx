...
import { AddKeyForm, AddModelForm, AccountIdForm, BaseUrlForm } from '@/components/admin/llm/LlmForms';
...
async function KeysSection({ providerId, providerSlug }: { providerId: string; providerSlug: string }) {
  const supabase = createSupabaseService();
  if (!supabase) return <p role="alert" className="text-sm text-red-700">Supabase not configured.</p>;
  ...
  return (
    <>
      ...
      <AddKeyForm providerId={providerId} providerSlug={providerSlug} />
    </>
  );
}
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
      ...
      <header className="mt-3">
        ...
        {accountId !== null ? (
          <AccountIdForm providerId={prov.id} defaultValue={accountId} />
        ) : null}
      </header>
      ...
      <section className="mt-6 rounded-xl border border-line bg-surface p-4 shadow-card">
        <Suspense fallback={<BoardSkeleton lines={3} label="Memuat key…" />}>
          <KeysSection providerId={prov.id} providerSlug={prov.slug} />
        </Suspense>
      </section>
    </div>
  );
