...
Format version: 1
Last updated: 2026-09-02 10:25 (local time)

## Current State

- **Status:** Content Factory selesai Fase 1–3 + P0/P1 audit + magic-link token_hash + admin quick-wins (dashboard/list/nav, profiles.is_admin single source) + **content pipeline 4-tahap (2 Sep)**: form → `content_research_sessions` → cron `asharu-content-research` (*/10) advance Discovery→Verification→Scoring→`awaiting_selection` (admin shortlist)→Development→draft. Search real-time via **Tavily**. Affiliate = top-20 terbaru + scoring relevansi + card produk di review (badge, Ganti/Hapus). Domain `https://asharu.id` live. **Factory MASIH berhenti (fail-closed)** menunggu setup P0 user (seed Vault `asharu_cron_secret` + Vercel `CRON_SECRET` + apply `20260831000001`) — kini juga gate pipeline riset. **`TAVILY_API_KEY` belum diset di Vercel** (user action). Sisa P1 (INSERT anon limit) + P2/P3 + iterasi discovery (single-pass dulu) belum.
...
- [ ] **`TAVILY_API_KEY` belum diset di Vercel Production** — tanpa ini, pipeline riset Discovery gagal (`getSearchProvider()` throw → session `failed`). User action: buat key di tavily.com → set di Vercel env. (Tidak perlu Vault; processor baca `process.env`.)
- [ ] **Pipeline riset belum E2E di production** — butuh: (1) `TAVILY_API_KEY` diset, (2) setup P0 cron (CRON_SECRET + Vault + apply `20260831000001`). Setelah itu submit form → pantau `/admin/riset`.
