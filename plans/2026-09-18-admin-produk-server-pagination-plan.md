...
- [ ] Buat file dengan (SEMUA murni, tanpa import supabase agar gampang di-test):
  ```ts
  ...
  ```
- [ ] JANGAN lupa koma di-escape: `.or()` PostgREST memakai koma sebagai pemisah kondisi — search `a,b` tanpa escape = 4 kondisi, bug senyap. Test khusus: `'100%, a_b,c\\d'` → semua karakter spesial ter-escape.
- [ ] Test `src/lib/admin/produk-query.test.ts`: parse valid/invalid/undefined; clamp (0, -3, NaN, 'abc', >total, total 0); range page 1 (`0-19`), page 3 (`40-59`); escape semua spesial; `buildSearchOr('')` → null.
- [ ] Acceptance: file test baru hijau via `npx vitest run src/lib/admin/produk-query.test.ts`.

### S2 — Query server di `page.tsx`

- [ ] Ubah `PageProps`: tambah `searchParams: Promise<{ page?: string; q?: string; filter?: string }>` (Next 15 async — JANGAN baca sinkron).
- [ ] Parse: `const pageRaw = ...; const q = (sp.q ?? '').trim().slice(0, 80); const filter = parseProdukFilter(sp.filter);`
- [ ] Query data (pertahankan order existing `:57-60`, GANTI `limit(500)`):
  ```ts
...
- [ ] `curatedCount` (`:66`): query terpisah `.select('id',{count:'exact',head:true}).eq('featured_override',true)` — JANGAN hitung dari halaman (salah).
- [ ] Sort JS `:62-64` DIHAPUS (order sudah di SQL; `neq is_active` gantikan filter). Tipe `AffiliateRow` (`:17-31`) tetap.
- [ ] Oper ke board props baru: `items, curatedCount, page, totalPages, totalCount, q, filter` (lihat S3 untuk bentuk pasti — tentukan interface dulu sebelum edit board).
- [ ] Bungkus board: `<Suspense fallback={<BoardSkeleton label={t('loading')} />}>` — import dari `@/components/admin/llm/ActionFeedback` (tiru `admin/llm/page.tsx:11,93`). `t('loading')` key baru S4.
- [ ] JANGAN ubah: guard `isAdmin` (`:50`), metadata (`:33-43`), shell/h1/intro (`:69-77`).

### S3 — Board baru (`FeaturedProductBoard.tsx`)

- [ ] HAPUS state `q`/`filter` (`:49-50`) + blok `filtered` (`:54-64`) + `.map(filtered)` → `.map(items)`. HAPUS `useState` untuk keduanya (tetap `useState` untuk `notice`/`busyId`).
- [ ] Filter bar (`:89-108`) → form GET biasa (tanpa JS submit):
  ```tsx
...
- [ ] Counter hasil DI ATAS list (ganti `curatedCount` line? TIDAK — `curatedCount` (`:90`) tetap; TAMBAH baris baru): `<p role="status" aria-live="polite">{t('rangeInfo', { from, to, count: totalCount })}</p>` — `from/to` dari props (hitung di server, oper sebagai props agar tak duplikasi rumus; atau oper `page,totalCount` dan hitung via `pageRange` import helper S1 — pilih yang kedua, satu sumber rumus).
- [ ] Pagination `<nav aria-label={t('paginationLabel')}>` SETELAH list (tiru `KontenList.tsx:342-364`): Prev hanya bila `page>1`, Next hanya bila `page<totalPages` (sembunyikan di ujung, JANGAN disable — pola `:347,355`). Link = `I18nLink` dari `@/i18n/navigation` (JANGAN `next/link` — prefix locale; pola `ReviewListClient.tsx:256-257`) dengan query `{...(`{q, filter}` bila non-default), page: String(page±1)}`. Tampilkan `t('pageOf', { page, total: totalPages })` di tengah.
...
- [ ] Empty: `filtered.length===0` → `items.length===0` + `t('empty')` (`:110-111` tetap, kondisi ganti).
- [ ] Props interface baru (TENTUKAN DULU, lalu sesuaikan S2):
  ```ts
  { items: ProductRow[]; curatedCount: number; page: number; totalPages: number; totalCount: number; q: string; filter: ProdukFilter }
  ```
  Import tipe `ProdukFilter` dari helper S1 (JANGAN definisi ulang `FilterMode` lokal bila sudah ada — cek baris ~24; gabungkan).
- [ ] `handleAction` (`:66-85`) + `router.refresh()` (`:78`) TAK BERUBAH (refresh me-load ulang halaman N saat ini — benar setelah pin/swap; catat di notice bila item pindah halaman? TIDAK perlu — cukup notice existing).

### S4 — i18n + test + gate

- [ ] Tambah di `admin.produk` id.json (`:808-834`) + mirror en.json (parity penuh — `messages.test.ts`):
...
- [ ] Test komponen (pola `ArticleCard.test.tsx`; mock `@/i18n/navigation` sudah di `vitest.setup.tsx`): render page 2/5 (link Prev→page1, Next→page3, query q+filter preserved); page 1 (tanpa Prev); page terakhir (tanpa Next); empty (t('empty')); tombol busy (disabled + spinner + `aria-busy`); notice sukses (`noticeSwapped`) dan error.
- [ ] Gate berurutan: `npm run typecheck` → `npm run lint` → `npm test` (penuh; flaky Studio pre-existing → rerun file spesifik) → `npm run build` (wajib: ubah RSC params + Suspense).
- [ ] Aturan insiden `prefer-const`: edit apa pun setelah hijau → re-run `typecheck + lint` sebelum commit.
...
- Belum mulai — S1 helper `produk-query.ts`.
