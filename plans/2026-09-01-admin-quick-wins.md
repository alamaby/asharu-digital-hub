...
- 2026-09-01 14:45 — Polish + digest `2948654141` fix SELESAI & ter-push. Bug ditemukan via Vercel log: `usePathname is not supported in Server Components` di `KontenList.tsx` (tanpa `'use client';`). Fix: tambah directive (1 baris, commit `7a0dffb`). Polish (commit `5cc6391`): dashboard "draf terbaru" jadi `<Link>` ke `/konten/review`; form "Buat Konten" success state inline dengan 2 link (Lihat draf → `/admin/konten?type=requests&status=pending`, Buat lagi reset); back nav "← Kembali ke Dasbor" + "Lihat semua" di header `/konten/baru`; loading.tsx skeleton di `/admin` & `/admin/konten`; error.tsx boundary di `/admin` & `/konten/review` (friendly fallback dengan Retry + Kembali ke Dasbor); approve/reject optimistic update + rollback + spinner SVG; filter form pakai `useFormStatus` spinner; pagination pakai `useTransition` + `window.location.href` (dengan spinner); vitest setup mock `next/navigation` & `@/i18n/navigation` agar tests jalan di vitest. Gate hijau (177/177, build ✓). Push: parent `017053d..7a0dffb..5cc6391`.
- 2026-09-01 16:25 — Fix i18n keys literal di list page + loading feedback. Root cause: namespace `admin` tidak masuk `CLIENT_MESSAGE_NAMESPACES` → `useTranslations('admin.konten')` di client component return key sebagai fallback. Tambah `'admin'` ke array. Bonus: ganti `useFormStatus` (tidak jalan untuk native GET form) → `useTransition` + `router.push` agar Apply & pagination punya pending state shared. Form: convert dari `<form action={pathname} method="get">` ke `<form onSubmit={...}>` client-handled. Pagination: dari `<Link>` ke `<button onClick={() => startTransition(router.push)}>`. Hardcoded "Terapkan"/"Reset" → `t('apply')`/`t('reset')`. Gate hijau (177/177, build ✓). Push: parent `1dc92b7..ed81738`.
- 2026-09-02 10:25 — (final) Placeholder "Riset & Topik" di dashboard kini punya halaman asli: `/admin/riset` (content pipeline 4-tahap, lihat `plans/2026-09-02-content-pipeline-4stage.md`). Quick-wins admin selesai; dashboard actions "Lihat Semua Konten"/"Review" tetap valid.
