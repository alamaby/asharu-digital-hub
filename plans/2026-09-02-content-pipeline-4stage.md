...
### FASE A — Fondasi
- [ ] Submodule migration `20260901000002_research_pipeline.sql`:
...
- [ ] Apply migration via MCP asharu.
- [ ] `src/lib/env.ts`: add `TAVILY_API_KEY` Zod (non-empty, prefix `tvly-`).
- [ ] `scripts/seed-research-keys.mjs`: Vault seeder (pattern: read `TAVILY_API_KEY` from env, else generate UUID placeholder, write to Vault via `public.vault_create_secret`).
- [ ] `src/lib/research/search.ts`: `SearchProvider` interface (`search(query, opts) → SearchResult[]`), `TavilyProvider` impl (POST `https://api.tavily.com/search` with API key, returns `{title, url, content, score, publishedDate}`), factory `getSearchProvider()`.

### FASE B — Modules
- [ ] `src/lib/llm/prompts/discovery.ts` — system prompt (user's spec verbatim) + JSON schema.
- [ ] `src/lib/llm/prompts/verification.ts` — per-batch fact-checker prompt.
- [ ] `src/lib/llm/prompts/scoring.ts` — batch scoring prompt + JSON schema.
- [ ] `src/lib/research/discovery.ts` — `runDiscovery(supabase, sessionId)`: build queries, parallel `tavily.search`, dedup by URL hostname+title, call LLM with raw search results, insert topics.
- [ ] `src/lib/research/verification.ts` — `runVerification(supabase, sessionId)`: per-topic targeted search, LLM verify date/location/source, update `verification_status`.
- [ ] `src/lib/research/scoring.ts` — `runScoring(supabase, sessionId)`: LLM score 8 sub-aspects + penalty + final_score per topic, update score_breakdown + final_score, rank.
- [ ] `src/lib/research/affiliate.ts` — `selectAffiliateProduct(supabase, topic)`: query last 20, score by category match (50) + keyword overlap (3/word ≥4 chars), pick top, attach `affiliate_match_score` + `signals`.
- [ ] `src/lib/research/development.ts` — `runDevelopment(supabase, sessionId)`: pick shortlisted topic, build thread prompt (reuse `buildThreadPrompt`), call LLM, select affiliate, insert `content_drafts` with `research_topic_id` + `affiliate_match_score` + `signals`.
- [ ] `src/lib/research/state-machine.ts` — pure functions: `nextStage(currentStatus)`, `canTransition(from, to)`, `validateParams(params)`.
- [ ] `src/lib/research/orchestrator.ts` — `advanceStage(sessionId)`: read session, switch on status, run stage handler, write results, transition status. Idempotent (if already in target stage, no-op). Catches errors → set status='failed' + log.

### FASE C — Processor rewrite
- [ ] `src/app/api/content/process/route.ts` rewrite:
  ...
  - `maxDuration = 300`.
- [ ] `src/app/api/content/process-legacy/route.ts` (new):
  - Auth same.
...
- [ ] `src/components/content/ContentRequestForm.tsx` — extend fields: target_location, secondary_location, audience_age, audience_interests (comma-separated → array), allowed_categories (multi-select), excluded_categories (multi-select), freshness_hours, minimum_candidates, minimum_score, required_winners, maximum_iterations. Collapsible "Pengaturan Riset Lanjutan" group.
- [ ] `src/lib/content/actions.ts`: `createContentRequest` → `createResearchSession(formData)`. Insert to `content_research_sessions`. Keep `createContentRequest` for legacy.
- [ ] `src/app/[locale]/konten/baru/page.tsx`: extend header copy.
- [ ] `src/messages/{id,en}.json`: keys for new fields.

### FASE E — Admin pages
- [ ] `src/app/[locale]/admin/riset/page.tsx` (RSC) — list sessions, filter by status + date, card per session.
- [ ] `src/app/[locale]/admin/riset/[sessionId]/page.tsx` (RSC) — detail: topics table, batch action "Lanjut ke Development".
- [ ] `src/app/[locale]/admin/riset/[sessionId]/topics/[topicId]/page.tsx` (RSC) — detail per topic.
- [ ] `src/app/[locale]/admin/riset/loading.tsx` + `error.tsx` (skeleton + boundary).
- [ ] `src/app/[locale]/admin/riset/[sessionId]/loading.tsx` + `error.tsx`.
- [ ] `src/config/navigation.ts`: `adminNavItems` tambah `{ key: 'adminRiset', pathname: '/admin/riset' }`.
- [ ] `src/i18n/routing.ts`: pathnames `/admin/riset`, `/admin/riset/[sessionId]`, `/admin/riset/[sessionId]/topics/[topicId]`.
- [ ] `src/messages/{id,en}.json`: `admin.research.*` namespace.
- [ ] `src/components/admin/ResearchSessionsList.tsx` + `ResearchSessionDetail.tsx` + `ResearchTopicDetail.tsx` (server components, mostly markup + Link to actions).
- [ ] `src/lib/content/actions.ts`: `shortlistTopics(sessionId, topicIds[])` + `rejectTopics(sessionId, topicIds[])` + `advanceToDevelopment(sessionId)` + `swapAffiliateProduct(draftId, newProductId)` + `removeAffiliateInjection(draftId)`. All `isAdmin()` guard.
- [ ] `src/components/content/AffiliateProductCard.tsx` (client) — image, name, category, merchant, URL, relevance badge, **Ganti** + **Hapus** buttons.
- [ ] `src/components/content/AffiliateProductPicker.tsx` (client) — modal/dropdown 20 produk terakhir + select action → `swapAffiliateProduct`.
- [ ] `src/components/content/ContentDraftCard.tsx` — extend dengan section "Produk Afiliasi" (render `AffiliateProductCard`).
- [ ] `src/messages/{id,en}.json`: `content.review.affiliate.*` namespace (relevanceHigh/Medium/Low, swap, remove, viewProduct, placeholderWarning, noRecentProducts).

### FASE F — Tests
- [ ] `src/lib/research/affiliate.test.ts` — last 20 pool, category match, no match fallback, empty pool, signals.
- [ ] `src/lib/research/scoring.test.ts` — formula deterministic, weight sum=1.
- [ ] `src/lib/research/discovery.test.ts` — dedup, candidate count.
- [ ] `src/lib/research/verification.test.ts` — status transitions.
- [ ] `src/lib/research/search.test.ts` — Tavily mock.
- [ ] `src/lib/research/state-machine.test.ts` — transitions valid/invalid.
- [ ] `src/lib/research/orchestrator.test.ts` — advanceStage idempotent.
- [ ] `src/app/api/content/process/route.test.ts` — auth + advance.
- [ ] `src/lib/content/actions.test.ts` (extend) — `swapAffiliateProduct` + `removeAffiliateInjection` + `shortlistTopics`.
- [ ] `src/components/content/AffiliateProductCard.test.tsx` (snapshots).

### FASE G — Gate + commit + push + docs
- [ ] Gate: lint + typecheck + test + build.
- [ ] Submodule commit: migration + cron config.
- [ ] Parent commits logical: (1) FASE A + B (backend), (2) FASE C + D (processor + form), (3) FASE E (admin UI), (4) FASE F (tests), (5) docs.
- [ ] Push.
- [ ] `.memory/2026-09-02/HHmmss-pipeline-4stage.md` + README.
- [ ] Update `plans/2026-09-01-admin-quick-wins.md` log (final).
