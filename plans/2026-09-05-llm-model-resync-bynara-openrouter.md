# Resync Model Bynara + OpenRouter

Created: 2026-09-05 11:00:00

## Objective
Samakan `llm_models` dengan 2 list user: 7 model Bynara (slug `naraya`) aktif dan 9 model OpenRouter aktif; semua model di luar list di-disable via `is_active=false` (non-destruktif, tanpa DELETE).

## Scope
- In: 1 migrasi SQL resync, verifikasi SELECT, gate (test/typecheck/lint/build), entry `.memory/`.
...
## Milestones
1. File plan ini
2. Migrasi `20260905000004_llm_models_resync.sql`
3. Verifikasi + gate hijau + memory entry
