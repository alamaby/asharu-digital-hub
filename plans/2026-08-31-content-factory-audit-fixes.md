...
- [x] **Push** (user approved 31 Agu ~10:00): submodule `dab480e..7bf4f30`, parent `e612f28..fcd0039`
- [x] Docs (plan + memory)
- [x] **3 baris `content_requests` processing nyangkut** → di-set `failed` (user decision, 31 Agu ~10:00; MCP `execute_sql`)
- [x] **Magic-link `token_hash` flow** (pekerjaan terpisah, 31 Agu ~10:05): template `magic_link.html` embed `{{ .TokenHash }}`+`{{ .Type }}` → exchange page `verifyOtp`; Flow 2 `?code=` legacy dipertahankan. Push: submodule `7bf4f30..721e6a1`, parent `fcd0039..2df08a6`. [USER STEP] Dashboard prod template.
...
- 2026-08-31 ~10:00 — User decision: (1) push, (2) 3 baris nyangkut → `failed`, (3) lanjut magic-link. Push Fase 2: submodule `dab480e..7bf4f30`, parent `e612f28..fcd0039`. UPDATE 3 baris `content_requests` status='processing' → 'failed' (via MCP `execute_sql`; ids cf8b8b5c5, 2972da62, e30761c7; attempts=0).
- 2026-08-31 ~10:05 — Magic-link `token_hash` flow SELESAI: `magic_link.html` href `{{ .ConfirmationURL }}` → `{{ .SiteURL }}/id/auth/exchange?token_hash={{ .TokenHash }}&type={{ .Type }}` (+ komentar HTML); exchange page Flow 1 `verifyOtp` (sesi sebelumnya) kini aktif; `LoginForm.tsx` komentar di-update. Gate hijau (lint ✓ typecheck ✓ test 173/173 ✓ build ✓). Push: submodule `7bf4f30..721e6a1`, parent `fcd0039..2df08a6`. [USER STEP PRODUKSI] Dashboard → Auth → Email Templates → Magic Link perlu paste template body sama (config.toml = local-only).
- 2026-08-31 ~10:10 — User konfirmasi template Supabase Dashboard (Auth → Email Templates → Magic Link) sudah diset. Flow `token_hash` aktif di produksi. Verifikasi direkomendasikan (belum dilakukan): request magic link ke admin email → link harus berisi `token_hash=` (bukan `/auth/v1/redirect`), klik → login sukses tanpa "PKCE code verifier not found".
- 2026-08-31 ~10:50 — **Bug produksi:** email bawa URL `http://localhost:3000/id/auth/exchange?token_hash=...` (bukan `https://asharu.id`). Root cause: template `{{ .SiteURL }}` resolve ke Dashboard "Site URL" setting (default `http://localhost:3000`); `config.toml` = local-only. Fix: template base URL `{{ .SiteURL }}/id/auth/exchange?...` → `{{ .RedirectTo }}?...` (app-controlled via `emailRedirectTo` = `${env.siteUrl}/id/auth/exchange`, environment-aware). 504 awal = port SMTP salah di config Supabase (user sudah fix). LoginForm comment di-update (redirect_to kini KRITIKAL). Gate hijau. Push: submodule `721e6a1..2a8215d`, parent `2df5762..0cab152`. **[USER STEP]** re-paste template ke Dashboard (template lama di Dashboard masih `{{ .SiteURL }}`). [HYGIENE opsional] Set Dashboard Site URL = `https://asharu.id`.
