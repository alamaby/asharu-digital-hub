...
- 2026-09-15 ~13:45 — Feedback user (screenshot): thumbnail inline 48px terlalu kecil. Keputusan via clarifikasi: ubah jadi figure full-width di atas teks (`max-h-96 object-contain` + caption nama produk), bukan `size-20`. Test inline diperkuat (figure + img + figcaption, tak ada img dalam `<p>`). Verifikasi runtime `next start`: HTTP 200, figure=1, size-12=0, ApplicationError=0. Gate: typecheck ✓ lint ✓ test 610/610 ✓ build ✓.
